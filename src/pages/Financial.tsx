...
export default function Financial() {
  ...
  const handleConfirmExport = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `relatorio-${exportInicio}-a-${exportFim}.xlsx`
      );
  ...
  // Função para formatar com segurança strings ISO de data
  const formatarData = (dataIso?: string) => {
    if (!dataIso) return "-";
    try {
      const parsed = typeof dataIso === "string" ? parseISO(dataIso) : new Date(dataIso);
          ...
          <div className="space-y-1">
            ...
            <h3 className="text-xl font-bold">
              {resumoLoading
                ? "..."
                : formatarMoeda(
                    resumo?.revenueToday ?? resumo?.faturamentoHoje
                  )}
            </h3>
          </div>
          ...
          <div className="space-y-1">
            ...
            <h3 className="text-xl font-bold">
              {resumoLoading
                ? "..."
                : formatarMoeda(
                    resumo?.revenueWeek ?? resumo?.faturamentoSemana
                  )}
            </h3>
          </div>
          ...
          <div className="space-y-1">
            ...
            <h3 className="text-xl font-bold text-amber-500">
              {resumoLoading
                ? "..."
                : formatarMoeda(
                    resumo?.faturamentoMes ?? resumo?.revenueMonth
                  )}
            </h3>
          </div>
          <TrendingUp className="text-amber-500 h-6 w-6" />
          ...
          <div className="space-y-1">
            ...
            <h3 className="text-xl font-bold">
              {resumoLoading
                ? "..."
                : formatarMoeda(
                    resumo?.faturamentoAno ?? resumo?.revenueYear
                  )}
            </h3>
          </div>
      ...
      <table className="w-full text-left border-collapse min-w-[600px]">
        <thead>
          <tr className="border-b border-border bg-muted/50 text-xs text-muted-foreground uppercase">
            <th className="p-4 font-medium w-1/6">Data</th>
            <th className="p-4 font-medium w-2/6">Cliente</th>
            <th className="p-4 font-medium w-2/6">Serviço</th>
            <th className="p-4 font-medium text-right w-1/6">Valor</th>
          </tr>
        </thead>
            ...
            recebimentosData.items.map((item: any, index: number) => (
              <tr key={item.id || index} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="p-4 text-muted-foreground">
                  {item.servico || "Serviço Realizado"}
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border w-full max-w-md rounded-2xl p-6 space-y-4 shadow-xl">
            ...
            <p className="text-xs text-muted-foreground">
              Por padrão, o sistema utiliza o período já filtrado na tela. Você
              também pode definir um valor mínimo opcional.
            </p>
