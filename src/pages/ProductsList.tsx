...
import { Plus, ChevronDown, ChevronRight, Trash2, Edit2, X, GripVertical, Layers, Image as ImageIcon } from "lucide-react";
...
export default function ProductsList() {
  ...
  // Estados dos formulários de Produtos (incluindo imagem)
  ...
  // Mutação para criar produto (suportando FormData para envio de imagem)
  const createProductMutation = useMutation({
    mutationFn: async (formData: FormData) => api.post("/products/items/", formData, {
  ...
  const updateProductMutation = useMutation({
    mutationFn: async ({ id, formData }: { id: string; formData: FormData }) => api.put(`/products/items/${id}`, formData, {
      headers: { "Content-Type": "multipart/form-data" }
    }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["products-list"] }); fecharModal(); }
  });
  ...
  const handleAbrirNovoProduto = (catId?: string) => {
    setCategoriaSelecionada(catId || (items[0]?.id.toString() ?? ""));
  ...
  const handleAbrirEditarProduto = (prod: Produto, catId: string) => {
    ...
    setImagemPreview(prod.imagemUrl || "");
    setImagemFile(null);
    setIsModalAberto(true);
  };
  ...
  const handleSalvarProduto = (e: React.FormEvent) => {
    ...
    if (editandoProdutoId) {
      updateProductMutation.mutate({ id: editandoProdutoId, formData });
    } else {
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerencie o catálogo de produtos e arraste para alterar a ordem de exibição</p>
        </div>
          ...
          <div className="bg-card border border-border w-full max-w-md rounded-xl shadow-2xl relative z-10 p-5 space-y-4">
            ...
            <form onSubmit={handleSalvarProduto} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-4">
                  <div className="h-16 w-16 rounded-lg bg-secondary border border-border flex items-center justify-center overflow-hidden relative flex-shrink-0">
                    {imagemPreview ? (
                  ...
                  <div className="flex-1">
                    <input type="file" accept="image/*" onChange={handleFileChange} className="w-full text-xs text-muted-foreground file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-primary file:text-primary-foreground hover:file:opacity-90 cursor-pointer" />
                  </div>
                </div>
              </div>
